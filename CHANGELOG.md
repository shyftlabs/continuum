...
- **Model output and user input that looked like labels.** Five values were left bare because of their names, and each held free text: the reflection critique's `verdict` (the critique model's whole reply, at `INFO`), `structured_output_error` (pydantic quotes the failing value as `input_value='…'`, which is the model's answer), a handoff's `reason` (what the model wrote about the user's request), an input `ValidationError`, and `extra={"preview": …}` in `runner.py` and `llm/client.py` (the first 100 characters of the model's answer). The verdict line now prints the SDK's own `PASS` / `NEEDS IMPROVEMENT` classification, which decides the retry, and withholds the reason.
- **`run_finalizer` logged `original[:8]` of a session id with no wrapper**, at `DEBUG`. On plaintext session ids those eight characters are `u:alice@`, the start of an email address.
- **Tool context reached agents that had no tools.** The system message listing captured tool values (`Current tool context …`, then `A session already exists …`) was added to every agent that inherited a non-empty context. In a live debate the two debaters and the judge each received it, so the cart id `<user_id>:<conversation_id>` went to the model provider three extra times, alongside instructions for tools they did not have. It now reaches only agents with regular tools. A handoff-only orchestrator counts as tool-less, since a handoff takes a reason, never a `session_id`.
...
- **`GeminiProvider` forwards `LLMConfig.extra_body`**, which it silently dropped. `BaseAgent` accepts the field and `LLMConfig.from_agent_config` carries it across, so an agent configured with provider options against Gemini got a request without them — no error, no warning, and the only symptom was the upstream behaving as if nothing had been configured. Both wire providers now `deepcopy` it: one `LLMConfig` serves many calls, pydantic's copy is shallow, and the dict was handed to the SDK by reference. Credit to PR #100, which found it.
- An unauthorized tool-approval attempt (`temporal`) now names the actor and withholds the approver roster, where before it printed both. The actor is named because identifying them is what a security audit line is for — pseudonymising it was tried and reverted, since the only route back to the real name is `LOG_PROMPT_CONTENT`, which reveals every prompt in the deployment and so would never be switched on to answer one question. The roster is withheld for a different reason: printing who *could* have approved, on the line that fires when someone is probing the approval gate, publishes the list of people to impersonate. The count is kept.
- **The TOOLS log line prints the tool list.** It had been withheld as `<1003 chars>` since the first redaction commit. The tool list holds each tool's name and parameter schema, defined before any user arrived, so by the "whose data is it?" rule it is the system's own fact. It stays at `INFO`. `FINAL PROMPT` on the same turn is still withheld.
- **`.env.template` lists `LOG_PROMPT_CONTENT=false`**, beside `LOG_LEVEL`.

### Fixed
- **One id, one pseudonym.** A live log showed one user as three unrelated pseudonyms on consecutive lines. `log_id()` of a `MemoryScope` or a dict pseudonymised its repr, and `log_id(session_id[:8])` pseudonymised a prefix. A mapping is now pseudonymised value by value with its keys kept, so the user inside it matches the user logged alone, and no logger argument in `src/` may be sliced (tested). `log_id(x[:8] if x else "none")` had also given every absent id the pseudonym of the word `none`.
- **`ToolContextConfig.inject_into_system_prompt` did nothing.** It was documented and defaulted `True`, but never read, so `False` still showed the server's values to the model. It is now honoured when the prompt is built, from the servers' current configs. Capture, and injection into tool arguments, are unchanged. See `docs/tools.md` §4.
- **A debate judge's partial view is reported.** With `summarise_arguments=False`, the default, each side is cut to its first `truncate_chars` characters, and nothing said so. In a live run the judge saw 49% and 58% of the two arguments, missing each one's conclusion. The cut is now logged at `INFO` when it removes anything, and names `summarise_arguments=True` as the alternative. `gateway-multi-agent-shop` uses that mode.
- **`create_debate_agent`, `create_scatter_agent` and `create_supervised_agent` are exported from `continuum.agent`**, as `docs/agent.md` showed and the other workflow factories already were. A test now checks every import in that doc.
...
### Security
- **A person can be required before a declared tool runs** (finding F7) — `AgentConfig.tool_approval` names tools by fnmatch pattern and `approval_handler` is the async callable asked to decide. The gate sits in `ToolExecutor.execute_tool_call` between the policy check and argument injection, so a reviewer sees the arguments the tool will actually receive — something neither the policy gate nor tool-trust can see. Three outcomes: approved, denied, and **deferred** ("nobody answered; this did not happen and can be asked again"), which only a handler may return. Every SDK failure path **denies**: a tool declared with no handler, a handler that raises, a handler that returns the wrong type, and a timeout. Nothing is remembered between runs — a cached approval would authorise an execution the reviewer never saw — so an application that wants that builds it from `run_id` and `arguments` on the request, where the idempotency rule can be its own.
- **Tool arguments are validated against the declared schema** (finding F5) — an MCP server's `inputSchema` was advertised to the model and then ignored at the call boundary, so the tool function body was the first thing to notice a wrong shape, usually by raising something unhelpful from three frames in. Worse, a function fails where the bad value is first *touched*, so anything it did before that line had already happened. `validate_arguments_against_schema` now enforces required keys, unknown keys, and top-level `type`/`enum` **before** the function is entered. Nested structure, empty `{}` property schemas, and open schemas (`additionalProperties: true`, or no `properties`) are deliberately not enforced: a partial validator that states its scope beats one whose gaps are found in production. `ToolArgumentError` is a `ToolError`, not an `MCPError` — the server did nothing wrong.
- **Long-term memory carries provenance** (finding F6) — an indirect-injection payload could get itself "remembered" and then replayed into every future session, laundering itself clean by crossing a session boundary. Whatever data labels a run carries are now stamped onto every memory row it writes (`_data_labels`), and recalling that row re-taints the reading run even on a turn that calls no tool. Prompt-level fencing is defence in depth here rather than the control: it was measured across four models and `gpt-4o-mini` obeyed a planted instruction inside every envelope and wording tried.
...
- **`AgentConfig.tool_approval`** and **`AgentConfig.approval_handler`**, with `ToolApprovalRequest` / `ToolApprovalDecision` / `ToolApprovalSettings` and `ToolApprovalError` (`continuum.agent.approval`). A handler is `async (ToolApprovalRequest) -> ToolApprovalDecision`; the SDK ships none, so the prompt, the modal, the Slack message or the ticket stays the application's.
- **`temporal_tool_approval()`** and **`temporal_approval_handler()`** (`continuum.temporal`) — reach a reviewer who will answer in an hour. Under Temporal a tool call runs inside an activity, which cannot touch workflow state, so these signal `request_tool_approval` and poll `get_approval_decision`, heartbeating so the activity is not killed mid-wait. The answer returns through the workflow's existing `submit_approval`, so the reviewer's side — the UI, `HumanInLoopManager`, the `is_authorized` allow-list — is unchanged and a tool approval is not a second, weaker door. The activity blocks *in place*, so work done before the gate is not repeated; it does **not** survive a worker restart, and an unreachable workflow degrades to `deferred` rather than denied.
- **`AgentMemoryConfig.on_labeled_recall`** — what to do when recall returns rows carrying provenance labels: `"fence"` (default, and what every existing deployment already does), `"drop"`, or `"block"`. Left an operator choice because which is right depends on who reviews and how fast: blocking is strongest, and also the one that turns a single planted row into an outage if nobody watches the queue.
- **`ToolArgumentError`** (`continuum.tools`).
...
- **`AgentMemoryConfig.pre_store_filter` is now a real gate.** mem0 fuses extraction and storage inside one `add()` call and exposes no extract-without-store path (checked in 1.0.11 and 2.0.19), so a rejected fact used to be *deleted after it had already been written*. Against Milvus that delete lost a race it could not win — mem0's `delete()` reads the row back first, Milvus hides recent writes behind `Bounded` consistency, and measured live the delete issued milliseconds after the write failed outright while the rejected fact stayed searchable for minutes. The gate now sits inside mem0's own `_create_memory`, so nothing is inserted to undo. Two consequences worth knowing: facts are offered **one at a time** (the batch is not known until extraction finishes), so a `list[str] -> list[str]` filter is called as `filter([fact])` and membership decides — returning a *rewritten* string counts as a rejection, because a filter is a gate and not a transformer. And it **fails closed**: a filter that raises has said nothing about the fact, so the fact is not written, and a broken filter therefore rejects everything in that turn. A memory provider that does not declare `pre_store_filter` cannot gate and falls back to delete-after-write with its race; that degradation is logged once rather than passed over in silence.
