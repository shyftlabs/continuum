#!/usr/bin/env python3
"""
Workflow Shop CLI — demonstrates all 9 multi-agent workflow modes.
...
Available modes:
  sequential  parallel  loop  scatter  supervised
  planner     debate    reflection  router
"""
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Workflow Shop — 9 multi-agent modes")
    parser.add_argument(
        "--mode", "-m",
    ...
    print("=" * 64)
    print(f"  Workflow Shop  —  mode: {mode.upper()}")
    print(f"  {description}")
    print("=" * 64)
    print()
    ...
    user_id = args.user or input("Enter user ID (or Enter for auto): ").strip() or f"user-{uuid.uuid4().hex[:8]}"
