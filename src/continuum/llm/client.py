...
class LLMClient:
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        policy_store/policy_subject/data_labels: optional data-label model-routing
        gate. When a policy store + subject are supplied, the run's active labels
        are folded into the policy subjects and the target model is checked as
        ``llm:<model>`` before any provider is selected; a deny raises
        ModelAccessDeniedError. Mirrors the tool-access gate.
        """
        effective_config = config or self.default_config
        ...
        # Data-label model-routing gate: a tainted run may be denied a model.
