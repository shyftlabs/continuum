...
| strict_security         | `AgentConfig(strict_security=True)` raises at construction if an agent has side-effectful tools but no `policy_store`/`access_policies` (otherwise a warning is logged). Makes the fail-open default visible |
